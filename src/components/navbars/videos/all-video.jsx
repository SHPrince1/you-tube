...
const AllVideos = () => {
  ...
  const [movieData, setMovieData] = useState(() => JSON.parse(sessionStorage.getItem('movieData')) || []); // Initialize movie data state with data from sessionStorage if available
  const [page, setPage] = useState(() => parseInt(sessionStorage.getItem('currentPage')) || 1); // Initialize page state with the current page from sessionStorage if available
